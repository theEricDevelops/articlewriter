...
async def create_prompt(prompt: PromptCreate, db: Session = Depends(db_manager.get_db)):
    db_prompt = Prompt(**prompt.model_dump(exclude={"provider_ids", "prompt_metadata"}))
    if prompt.provider_ids:
        for provider_id in prompt.provider_ids:
            db_prompt.providers.append(Provider(
...
async def read_prompts(skip: int = 0, limit: int = 100, db: Session = Depends(db_manager.get_db)):
    prompts = db.query(Prompt).offset(skip).limit(limit).all()
    return prompts

@router.get("/{prompt_id}", response_model=PromptResponse)
async def read_prompt(prompt_id: int, db: Session = Depends(db_manager.get_db)):
...
async def update_prompt(prompt_id: int, prompt_update: PromptUpdate, db: Session = Depends(db_manager.get_db)):
    db_prompt = db.query(Prompt).filter(Prompt.id == prompt_id).first()
    if db_prompt is None:
        raise HTTPException(status_code=404, detail="Prompt not found")
    update_data = prompt_update.dict(exclude_unset=True)
    if "provider_ids" in update_data or "prompt_metadata" in update_data:
        db.query(Provider).filter(Provider.prompt_id == prompt_id).delete()
...
async def delete_prompt(prompt_id: int, db: Session = Depends(db_manager.get_db)):
    db_prompt = db.query(Prompt).filter(Prompt.id == prompt_id).first()
    if db_prompt is None:
        raise HTTPException(status_code=404, detail="Prompt not found")
    db.delete(db_prompt)
