...
from app.schemas.outlines import OutlineCreate, OutlineUpdate, OutlineResponse
import json

router = APIRouter()

@router.post("/", response_model=OutlineResponse)
async def create_outline(outline: OutlineCreate, db: Session = Depends(db_manager.get_db)):
    db_outline = Outline(
        structure=json.dumps(outline.structure),
        outline_metadata=json.dumps(outline.instructions.model_dump()) if outline.instructions else None,
...
async def read_outlines(skip: int = 0, limit: int = 100, db: Session = Depends(db_manager.get_db)):
    outlines = db.query(Outline).offset(skip).limit(limit).all()
    return outlines

@router.get("/{outline_id}", response_model=OutlineResponse)
async def read_outline(outline_id: int, db: Session = Depends(db_manager.get_db)):
...
async def update_outline(outline_id: int, outline_update: OutlineUpdate, db: Session = Depends(db_manager.get_db)):
...
async def delete_outline(outline_id: int, db: Session = Depends(db_manager.get_db)):
    db_outline = db.query(Outline).filter(Outline.id == outline_id).first()
    if db_outline is None:
        raise HTTPException(status_code=404, detail="Outline not found")
